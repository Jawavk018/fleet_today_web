...
export class MenuService {
  ...
  public createVerticalMenuItem(menu: Array<Menu>, menuItem) {
    ...
    this.renderer2.setAttribute(div, 'id', 'menu' + menuItem.appMenuSno);
    ...
    this.renderer2.setAttribute(link, 'id', 'link' + menuItem.appMenuSno);
    ...
    if (menuItem.routerLink) {
      this.renderer2.listen(link, "click", () => {
        let navigationExtras: any = {
          state: {
            isAdmin: menuItem.isAdmin
          }
        };
        this.router.navigate([menuItem.routerLink], navigationExtras);
    ...
    if (menuItem.hasSubMenu) {
      ...
      this.renderer2.setAttribute(link, 'href', '#collapse' + menuItem.appMenuSno);
      let collapse = this.renderer2.createElement('div');
      this.renderer2.setAttribute(collapse, 'id', 'collapse' + menuItem.appMenuSno);
      this.renderer2.setAttribute(collapse, 'data-parent', '#menu' + menuItem.parentMenuSno);
      this.renderer2.addClass(collapse, 'collapse');
      this.renderer2.appendChild(div, collapse);
      this.createSubMenu(menu, menuItem.appMenuSno, collapse, 'vertical');
    }
    return div;
  }
  ...
  public createHorizontalMenuItem(menu: Array<Menu>, menuItem) {
    ...
    if (menuItem.hasSubMenu) {
      ...
      this.createSubMenu(menu, menuItem.appMenuSno, subMenu, 'horizontal');
    }
    return li;
  }

  private createSubMenu(menu: Array<Menu>, menuItemId, parentElement, type) {
    let menus = menu.filter(item => item.parentMenuSno === menuItemId);
  ...
  public getActiveLink(menu: Array<Menu>) {
    ...
    if (activeMenuItem[0]) {
      let activeLink = document.querySelector("#link" + activeMenuItem[0].appMenuSno);
      return activeLink;
    }
    return false;
  }
  ...
  public setActiveLink(menu: Array<Menu>, link) {
    if (link) {
      menu.forEach((menuItem) => {
        let activeLink = document.querySelector("#link" + menuItem.appMenuSno);
        if (activeLink) {
    ...
    if (activeMenuItem[0]) {
      ...
      while (this.renderer2.parentNode(parent)) {
        ...
        if (parent.classList.contains('collapse')) {
          let parentMenu = menu.filter(item => item.appMenuSno === activeMenuItem[0].parentMenuSno);
          let activeParentLink = document.querySelector("#link" + parentMenu[0].appMenuSno);
  ...
  public addNewMenuItem(menu: Array<Menu>, newMenuItem, type) {
    ...
    if (newMenuItem.parentMenuSno != 0) {
      let parentMenu = menu.filter(item => item.appMenuSno === newMenuItem.parentMenuSno);
